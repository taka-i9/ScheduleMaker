...
<script>
    ...
    window.onload = function() {
        ...
        <?php
        if(count($errors) != 0) echo 'changeEdit("'.old('template_name').'", "'.old('name').'", "'.old('deadline_date').'", "'.old('deadline_time').'", "'.old('type').'", "'.old('required_hour').'", "'.old('required_minute').'", "'.old('repetition').'", "'.old('memo').'", "'.old('priority_level').'", "'.old('color').'")';
    ...
    function backList() {
        ...
        document.getElementById('form').action = "{{ route('todo.list') }}";
    ...
    function changeEdit(template_name, name, deadline_date, deadline_time, type, required_hour, required_minute, repetition, memo, priority_level, color) {
        ...
        repetition_value_content_everyday_checkbox.onchange = 'changeStateReptationEveryday(this)';
